...
public class J48 extends Classifier implements OptionHandler, Drawable,
	...
	/** The decision tree */
	public j48.ClassifierTree m_root;
	...
	public void buildClassifier(Instances instances) throws Exception {
		...
		j48.ModelSelection modSelection;

		if (m_binarySplits)
			modSelection = new j48.BinC45ModelSelection(m_minNumObj, instances);
		else
			modSelection = new j48.C45ModelSelection(m_minNumObj, instances);
		if (!m_reducedErrorPruning)
			m_root = new j48.C45PruneableClassifierTree(modSelection,
					!m_unpruned, m_CF, m_subtreeRaising, !m_noCleanup);
		else
			m_root = new j48.PruneableClassifierTree(modSelection, !m_unpruned,
					m_numFolds, !m_noCleanup, m_Seed);
		m_root.buildClassifier(instances);
		if (m_binarySplits) {
			((j48.BinC45ModelSelection) modSelection).cleanup();
		} else {
			((j48.C45ModelSelection) modSelection).cleanup();
		}
	}
