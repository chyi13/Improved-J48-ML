...
public class C45Split extends ClassifierSplitModel {
	 ...
	 */
	private double rrrrr = 0.0, lllll = 0.0;
	...
	private void handleNumericAttribute(Instances trainInstances)
		...
		while (next < firstMiss) {
			...
			if (trainInstances.instance(next - 1).value(m_attIndex) + 1e-5 < trainInstances
				...
				if (Utils.grOrEq(m_distribution.perBag(0), minSplit)
					...
					currentInfoGain = infoGainCrit.splitCritValue1(
							m_distribution, m_sumOfWeights, defaultEnt,rrrrr);
		...
		m_gainRatio = gainRatioCrit.splitCritValue1(m_distribution,
				m_sumOfWeights, m_infoGain,lllll);
	}
